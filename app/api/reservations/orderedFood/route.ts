...
export const POST = async (req: NextRequest) => {
    ...
    if(!reservation_id || !dish_id) return NextResponse.json({message: "All fields are required"},{status: 401})
    await connectToDB()
   try {
      const orderedFood = await orderedDish.create({reservation_id, dish_id}) as OrderedFoodType
   ...
   return NextResponse.json({orderedFood: newestOrderedFood}, {status: 201})
   } catch (error) {
