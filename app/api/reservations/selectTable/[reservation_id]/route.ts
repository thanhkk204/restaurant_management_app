...
export async function PATCH(req: NextRequest, { params }: { params: { reservation_id: string }}){
  console.log('thanh')
    ...
    await connectToDB()
   try {
   const oldReservation = await reservation.findById(reservation_id) as ReservationType
   ...
   if(type === "SELECT"){
    // update selected table status to ISSERVING
      ...
      await reservation.findByIdAndUpdate({_id: reservation_id}, {table_id: table_id,status: "SEATED", startTime: new Date()}, {new: true})
    }else if(type === "RESELECT"){
        // update privious table tatus to AVAILABLE
      await table.findByIdAndUpdate(oldReservation.table_id, {status: "AVAILABLE"})
      await table.findByIdAndUpdate(table_id, {status: "ISSERVING"})
       //  update reservation table_id
      await reservation.findByIdAndUpdate({_id: reservation_id}, {table_id: table_id,}, {new: true})
   }
