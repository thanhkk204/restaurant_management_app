...
export async function PATCH(
  ...
  if (!id)
    return NextResponse.json(
      { message: "There is no Id to update location" },
      { status: 401 }
    )
  if (!locationInRestaurant)
    return NextResponse.json(
      { message: "There is no body to update location" },
      { status: 401 }
    )
  ...
  try {
    ...
    return NextResponse.json(
      { message: "Update Successfully!", newLocation },
      { status: 201 }
    )
  } catch (error) {
...
export async function GET(
  ...
  if (!id)
    return NextResponse.json(
      { message: "There is no Id to get detail location" },
      { status: 401 }
    )
...
export async function DELETE(
  ...
  if (!id)
    return NextResponse.json(
      { message: "There is no Id to delete detail location" },
      { status: 401 }
    )
