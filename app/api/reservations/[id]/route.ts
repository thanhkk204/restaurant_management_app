import { DistricType, ProvinceType, ReservationType, WardType } from "@/lib/constants/type"
...
export const GET = async (req: NextRequest, {params}: {params: {id:string}}) => {
    ...
    if(!reservation_id) return NextResponse.json({message: "Missing reservation_id"}, {status: 401})
      await connectToDB()
   try {
    const reservationDetail = await reservation.findById(reservation_id).populate('addres_id').populate('table_id').populate('user_id')
     if(!reservationDetail) return NextResponse.json({message: "Can't find reservation"}, {status: 401})
   ...
   //  1: get list of locations 
    ...
    const districts = await getLocation<DistricType[]>('https://vapi.vnappmob.com/api/province/district/'+ province_id)
    const wards = await getLocation<WardType[]>('https://vapi.vnappmob.com/api/province/ward/'+ district_id)
    // 2: find exactly location
    const prov = (await provinces).find(item=> item.province_id === province_id)
    const dist = (await districts).find(item=> item.district_id === district_id)
