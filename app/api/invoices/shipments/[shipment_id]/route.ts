...
export async function POST(req: NextRequest, { params }: { params: { shipment_id: string }}){
    const {shipment_id} = params
    if(!shipment_id) return NextResponse.json({message: "There is no ID for create ordered"}, {status: 500})

    await connectToDB()
   try {
   const shipmentInfor = await shipment.findById(shipment_id).populate('addres_id') as ShipmentType
   await dish.find({})
   const orderedDishes = await orderedDish.find({shipment_id: shipment_id}).populate('dish_id') as OrderedFoodType[]
   ...
   const GHN_BODY = {
    ...
    //     ]
    // }
 if(!process.env.GHN_URL_CREATE || !process.env.GHN_TOKEN || !process.env.GHN_SHOP_ID) return NextResponse.json({message: "Something went wrong with variant .env"}, {status: 500})
 const GHN_Order = await fetch('https://dev-online-gateway.ghn.vn/shiip/public-api/v2/shipping-order/create',{
     method: "POST",
     headers: {
   ...
   if(res.message === "Success"){
    await shipment.findOneAndUpdate({_id:shipment_id},{order_code: res.data.order_code,status: "DELIVERING"}, {new: true})
   }
