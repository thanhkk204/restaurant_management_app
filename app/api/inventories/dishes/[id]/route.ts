...
export async function PATCH(
  ...
  if (!id)
    return NextResponse.json(
      { message: "There is no Id to update dish" },
      { status: 500 }
    )
  if (!body)
    return NextResponse.json(
      { message: "There is no body to update dish" },
      { status: 500 }
    )
  ...
  try {
    ...
    return NextResponse.json(
      { message: "Update Successfully!", newDish },
      { status: 201 }
    )
  } catch (error) {
...
export async function GET(
  ...
  if (!id)
    return NextResponse.json(
      { message: "There is no Id to get detail dish" },
      { status: 500 }
    )
